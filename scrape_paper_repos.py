...
def main():
    ...
    p.add_argument("path_in", help="path of text file containing one URL per line")
    p.add_argument("path_out", help="path of output file")
    args = p.parse_args()
    ...
    with open(args.path_in) as f_in, open(args.path_out, 'w') as f_out:
        for line in f_in:
            url = line.rstrip()
            meta = None
